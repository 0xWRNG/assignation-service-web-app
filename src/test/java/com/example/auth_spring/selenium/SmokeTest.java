...
public class SmokeTest {
    ...
    public void checkValidServiceCreation() {
        ...
        servicePage.enterTitle("Чистка зубов")
    ...
    @Test
    @Order(103)
    @DisplayName("1.3 Название 'AB'")
    public void checkShortTitleService() {
        loginAsManager();
        ServicePage servicePage = new ServicePage(driver).open(BASE_URL, 3);
        driver.manage().timeouts().implicitlyWait(Duration.ofSeconds(1));
        servicePage.enterTitle("AB")
                .enterDuration("30")
                .clickSaveExpectingFailure();
    ...
    @Test
    @Order(104)
    @DisplayName("1.8 Длительность 0")
    ...
    @Test
    @Order(105)
    @DisplayName("1.14 Описание 2000 сим")
    public void checkLongDescriptionService() {
        loginAsManager();
        ServicePage servicePage = new ServicePage(driver).open(BASE_URL, 3);
        driver.manage().timeouts().implicitlyWait(Duration.ofSeconds(1));
        servicePage.enterTitle("Длинное описание")
    ...
    @Test
    @Order(201)
    @DisplayName("2.1 Валидная регистрация USER")
    ...
    public void checkShortPasswordRegistration() {
        RegisterPage registerPage = new RegisterPage(driver).open(BASE_URL);
        driver.manage().timeouts().implicitlyWait(Duration.ofSeconds(1));
        registerPage.enterLogin("validlogin_" + System.currentTimeMillis())
                .enterPassword("weak")
                .enterEmail("abc@example.com")
                .enterPhone("+01234567890")
                .enterName("Name")
    ...
    @Test
    @Order(205)
    @DisplayName("2.10 Регистрация MANAGER")
    ...
    @Test
    @Order(301)
    @DisplayName("3.1 Бронирование доступного слота в будущем")
    public void checkValidBooking() {
        loginAsUser();
        String futureDate = LocalDate.now().plusDays(1).format(DateTimeFormatter.ofPattern("dd.MM.yyyy"));
        BookingPage bookingPage = new BookingPage(driver).open(BASE_URL, 3, 5);
        driver.manage().timeouts().implicitlyWait(Duration.ofSeconds(1));
        bookingPage.selectDate(futureDate)
                .selectExecutor()
                .selectTimeslot();
    ...
    @Test
    @Order(302)
    @DisplayName("3.4 Бронирование в прошлом")
    public void checkPastBooking() {
        ...
        BookingPage bookingPage = new BookingPage(driver).open(BASE_URL, 3, 5);
        driver.manage().timeouts().implicitlyWait(Duration.ofSeconds(1));
        bookingPage.selectDate(pastDate)
                .selectExecutor()
                .selectTimeslot();
        bookingPage.clickSubmit();
    ...
    @Test
    @Order(401)
    @DisplayName("4.1 Статус: Неподтвержденная -> Подтвержденная")
    ...
    @Test
    @Order(402)
    @DisplayName("4.2 Статус: Неподтвержденная -> Отмененная")
